        ...
        switch (req.method) {
            ...
            case 'PUT' :
                if(!isNaN(id)){
                    ...
                    if(validStatus === ''){
                        for(let bookmark of bookmarks){
                            if(bookmark.Id === id){
                                ...
                                updatedBookmarks.push(modifiedBookmark);
                                isFound = true;
                            } else {
                        ...
                        if(!isFound){
                            res.writeHead(404)
                            res.end("The bookmark with id : " + id + " doesn't exist.");
                        } else {
                        fs.writeFileSync(bookmarksFilePath, JSON.stringify(updatedBookmarks));
                        res.writeHead(201, { 'content-type': 'application/json' });
                        res.end(JSON.stringify(modifiedBookmark));  
                        }
                    } else {
                        res.writeHead(400);
                        res.end("Error : "+ validStatus);
                    ...
                    res.writeHead(400);
                    res.end("You must provides an id to use the put function");
                }
                break;
            case 'DELETE' :
                if(!isNaN(id)){
                    ...
                    res.writeHead(400);
                    res.end("You must provides an id to use the delete function");
                }
                break;
            case 'PATCH':
                res.writeHead(501);
